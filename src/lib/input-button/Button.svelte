...
<script lang="ts">
	...
	import type { ButtonVariant } from './input-button.types';
...
<input-button {variant}>
	{#if href && !disabled}
		<a href={href.toString()} on:click {...$$restProps}>
			<slot />
		</a>
	{:else}
		<button role={href ? 'link' : undefined} {type} {disabled} on:click {...$$restProps}>
			<slot />
		</button>
	{/if}
</input-button>
